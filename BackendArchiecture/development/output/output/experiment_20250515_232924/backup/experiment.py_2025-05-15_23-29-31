...
X_imputed_linear = imputer_linear.fit_transform(X)
X_imputed_linear = pd.DataFrame(X_imputed_linear, columns=X.columns)  # Convert back to DataFrame


# 5-fold cross-validation for Linear Regression
kf_linear = KFold(n_splits=5, shuffle=True, random_state=42)
cv_scores_linear = cross_val_score(linear_model, X_imputed_linear, y, cv=kf_linear, scoring='neg_mean_squared_error')

# Convert scores to RMSE
rmse_scores_linear = np.sqrt(-cv_scores_linear)

# Report the average RMSE
print("Average RMSE from 5-fold cross-validation (Linear Regression):", rmse_scores_linear.mean())